      ...
      <div className="relative mx-auto grid max-w-6xl grid-cols-1 gap-10 px-4 py-20 md:grid-cols-12 md:gap-12 md:px-6 md:py-28 lg:py-32">
        <div className="md:col-span-8">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-card/60 px-3 py-1 font-mono text-xs text-muted-foreground">
            ...
            <span>{hero.shippingLabel}</span>
            <span className="text-foreground">{hero.shippingProject}</span>
          </div>
          ...
          <p className="mt-4 font-mono text-sm text-muted-foreground sm:text-base">
            <span className="text-primary">{">"}</span> {hero.role}
        ...
        <aside className="md:col-span-4">
          <div className="rounded-lg border border-border bg-card/80 p-5 font-mono text-xs shadow-lg shadow-black/20">
            <div className="flex items-center gap-1.5 border-b border-border pb-3">
              ...
              <span className="ml-2 text-[11px] text-muted-foreground">{hero.aboutFileName}</span>
            </div>

            <pre className="mt-4 whitespace-pre-wrap leading-relaxed text-muted-foreground">
              <span className="text-muted-foreground/60">{"{"}</span>
              {"\n  "}
              <span className="text-foreground">"{hero.about.roleKey}"</span>
              <span>: </span>
              <span className="text-primary">"{hero.about.roleValue}"</span>
              <span>,</span>
              {"\n  "}
              <span className="text-foreground">"{hero.about.focusKey}"</span>
              <span>: [</span>
              <span className="text-primary">"{hero.about.focusValues[0]}"</span>
              <span>, </span>
              <span className="text-primary">"{hero.about.focusValues[1]}"</span>
              <span>, </span>
              <span className="text-primary">"{hero.about.focusValues[2]}"</span>
              <span>],</span>
              {"\n  "}
              <span className="text-foreground">"{hero.about.shippingKey}"</span>
              <span>: </span>
              <span className="text-primary">"{hero.about.shippingValue}"</span>
              <span>,</span>
              {"\n  "}
              <span className="text-foreground">"{hero.about.reviewsKey}"</span>
              <span>: </span>
              <span className="text-primary">{hero.about.reviewsValue}</span>
              <span>,</span>
              {"\n  "}
              <span className="text-foreground">"{hero.about.statusKey}"</span>
              <span>: </span>
              <span className="text-primary">"{hero.about.statusValue}"</span>
              {"\n"}
              <span className="text-muted-foreground/60">{"}"}</span>
            </pre>
